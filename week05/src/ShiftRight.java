...
public class ShiftRight {
    public static void main(String[] args){
        ...
        if (inputCounter){
            ...
            //System.out.println("array before shifting: "+Arrays.toString(array));
            //System.out.println("array after shifting: "+Arrays.toString(shiftRight(array, N)));
    ...
    public static int[] shiftRight(int[] array, int N){
        ...
        for (int i=0; i<tempArray.length; i++){ //array to temporarily store the elements shifted after the shift no
           tempArray[i]=array[array.length-count];
           count--;
        }
       //System.out.println("tempArray: "+Arrays.toString(tempArray));
        for (int j=0; j<array.length-N; j++){ //first array to store the shifted elements before shift number
        ...
        for (int k=0; k<tempArray.length; k++){//adding the element of first temp array to final array
            checkArray[k]=tempArray[k];
        }

        return checkArray;
    }

    //literally spend hours coding this. I know it a really vague and not efficient, but I did the best i can!
